...
//import java.util.Map;      // 🔑 回傳簡單的 JSON 用（Map.of(...)）
//
...
//import org.springframework.web.bind.annotation.CookieValue;  // 🍪 從請求 Cookie 直接取值的注解
...
//
//
//    public AuthRestController(AuthenticationManager am, JwtUtil jwt) {
//        this.am = am;
//        this.jwt = jwt;
//    }
//
//    // ===================== 登入 =====================
//    @PostMapping("/login") // 📮 POST /api/auth/login：接帳密，成功後下發 JWT（Cookie 版）
...
//        
//        String access  = jwt.generateAccess(auth.getName(), SecurityConfig.ACCESS_TTL);
//        String refresh = jwt.generateRefresh(auth.getName(), SecurityConfig.REFRESH_TTL);
...
//        ResponseCookie accessCookie = ResponseCookie.from(SecurityConfig.ACCESS_COOKIE, access)
...
//            .maxAge(SecurityConfig.ACCESS_TTL) // ⏰ 15 分鐘
//            .build();
//
...
//        ResponseCookie refreshCookie = ResponseCookie.from(SecurityConfig.REFRESH_COOKIE, refresh)
//            .httpOnly(true)
//            .secure(true)
//            .sameSite("Strict") // 🔒 比 Lax 更嚴：避免第三方導流時夾帶刷新
//            .path("/")
//            .maxAge(SecurityConfig.REFRESH_TTL) // ⏰ 7 天
//            .build();
//
