...
/**
 * MemberService
 * 職責：商業規則與流程協調（正規化、唯一性預檢、欄位更新範圍）。
 * 注意：不做持久化細節（交由 MemberDAO），不做雜湊（上層先把明碼轉成 hash）。
 */
@Service
public class MemberService implements MemberService_Interface {
    ...
    public Integer register(MemberVO member) {
        Objects.requireNonNull(member, "member 不可為 null");

        // ---- 正規化（屬 Service 範圍，不塞進 DAO/Entity）----
        member.setEmail(normEmail(member.getEmail()));
        member.setNickname(safeTrim(member.getNickname()));
        member.setFullName(safeTrim(member.getFullName()));
        member.setPhoneNumber(safeTrim(member.getPhoneNumber()));
        member.setAddress(safeTrim(member.getAddress()));

        // ---- 基本驗證（必要欄位）----
        requireNotBlank(member.getEmail(), "email 不可為空");
        requireNotBlank(member.getPasswordHash(), "passwordHash 不可為空");
        requireNotBlank(member.getFullName(), "fullName 不可為空");
        requireNotBlank(member.getNickname(), "nickname 不可為空");
        ...
        // ---- 寫入 ----
        try {
            dao.insert(member); // 回填 IDENTITY 主鍵
    ...
    public void updateProfile(MemberVO src) {
        ...
        if (db == null) throw new IllegalArgumentException("找不到該會員");

        // 限定可更新欄位（不含 email、passwordHash）
        if (src.getFullName() != null) db.setFullName(safeTrim(src.getFullName()));
        if (src.getGender() != null)   db.setGender(src.getGender());
        if (src.getBirthDate() != null) db.setBirthDate(src.getBirthDate());
        if (src.getPhoneNumber() != null) db.setPhoneNumber(safeTrim(src.getPhoneNumber()));
        if (src.getAddress() != null)  db.setAddress(safeTrim(src.getAddress()));
        if (src.getProfilePic() != null) db.setProfilePic(src.getProfilePic());
    ...
    public void changePassword(Integer memberId, String newPasswordHash) {
        if (memberId == null) throw new IllegalArgumentException("memberId 不可為 null");
