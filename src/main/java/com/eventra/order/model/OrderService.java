...
public class OrderService {
	...
	private static final String PROXY_HOST = "https://286b1b988565.ngrok-free.app";
	...
	public OrderService(ECPayUtils ECPayUtils, ExhibitionRepository exhibitionRepository, CartItemRedisRepository cartItemRedisRepository, OrderRepository orderRepository, OrderItemRepository orderItemRepository, PaymentAttemptRepository paymentAttemptRepository) {
...
/* ********* 3rd part : 庫存釋放 ********* */
		Map<Integer, Integer> releaseMap = 
				expiredOrders.stream().flatMap(order -> order.getOrderItems().stream())
				.collect(Collectors.groupingBy(item -> item.getExhibitionTicketType().getExhibitionId(), Collectors.summingInt(i -> 1)));
	...
	public String checkOrderStatus(String merchantTradeNo) {
		String orderStatus = PAYMENT_ATTEMPT_REPO.findByMerchantTradeNo(merchantTradeNo).orElseThrow().getOrder().getOrderStatus();
		return orderStatus;
	}
...
/* ********* 3rd part : 核對 CheckMacValue 失敗就在此截斷 0|FAIL ********* */
		...
		try{checkMacValue = ECPAY_UTILS.genCheckMacValue(ECPAY_UTILS.genCheckMap(req));}
		catch(Exception e) {System.out.println(e.toString());}
		System.out.println("merchantCheckMacValue: " + req.getCheckMacValue());
		System.out.println("checkMacValue: " + checkMacValue);
		
		if(!Objects.equals(req.getCheckMacValue(),checkMacValue)) return "0|failed";
...
/* ********* 6th??? part : 處理付款異常情況 ********* */ // Could be Phase II???
	...
	public ECPaySendingResDTO ECPaySending(ECPaySendingReqDTO req, Integer memberId) {
		...
		/* ********* 先算出所需參數 ********* */
		String itemName = vos.stream().map(vo -> vo.getExhibitionName() + "   " + vo.getTicketTypeName() + ": " + vo.getQuantity() + " 筆").collect(Collectors.joining("#"));
		...
		OrderVO orderVO = ORDER_REPO.save(
					new OrderVO.Builder()
					.orderUlid(ulid)
					.orderStatus("待付款")
		...
		List<OrderItemVO> orderItemVOs = vos.stream()
				.flatMap(vo -> IntStream.range(0, vo.getQuantity()) 
					// map [1,2] [0,1,2] ; flatMap [1,2,0,1,2]
				.mapToObj(i -> {
					// 把 IntStream / DoubleStream 基元型 stream 轉為 物件流
					OrderItemVO item = new OrderItemVO.Builder()
							.order(orderVO)
							.orderItemUlid(ulid + "-" + vo.getCartItemId() +(i+1))
							.exhibitionTicketType(entityManager.getReference(
								ExhibitionTicketTypeVO.class, vo.getExhibitionTicketTypeId()))
		...
		ECPaySendingResDTO res = new ECPaySendingResDTO.Builder()
				.status("success")
				.action("https://payment-stage.ecpay.com.tw/Cashier/AioCheckOut/V5")
				.method("POST")
				.fields(fieldsBuilder(merchantTradeNo, merchantTradeDate, totalAmount, itemName))
				.build();
		/* ********* 5th part : 新增 payment attempt 並回傳 ********* */
			// 需要 order, merchantTradeNo, merchantId, tradeAmt, paymentType, checkMacValue
		PaymentAttemptVO paymentAttemptVO = new PaymentAttemptVO.Builder()
...
//				.paymentType("Credit") // Callback 寫不一樣的...
	...
	private String genTicketCode(String ulid) {
		return "TK-" + ulid.substring(13,20) + "-" + new SimpleDateFormat("yyyyMMdd").format(new Date()) + "-" + ThreadLocalRandom.current().nextInt(1, 9999);
	}
	private Map<String, String> fieldsBuilder(String merchantTradeNo, String merchantTradeDate, Integer totalAmount, String itemName){
...
//		fields.put("OrderResultURL", "http://localhost:8088/order/ECPay/OrderResultURL");
		...
		try {checkMac = ECPAY_UTILS.genCheckMacValue(fields);} 
		catch (Exception e) {System.out.println(e.toString());}
