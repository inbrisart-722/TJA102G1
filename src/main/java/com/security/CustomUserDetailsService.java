...
public class CustomUserDetailsService implements UserDetailsService {
	...
	public CustomUserDetailsService(MemberRepository memberRepo, ExhibitorRepository exhibitorRepo) {
	...
	// 4. ExceptionTranslationFilter 再把它丟給 AuthenticationEntryPoint → 前端只會收到「Bad
	// credentials」或 401。
	...
	// 丟 UsernameNotFoundException 可以乾淨地中斷流程，交給 ExceptionTranslationFilter →
	// AuthenticationEntryPoint 處理。
...
//                .build();
	...
	// UserDetails 介面裡有一個方法：Collection<? extends GrantedAuthority> getAuthorities();
	// // 這就是用來裝「角色 / 權限」的。
