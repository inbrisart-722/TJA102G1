...
//import org.springframework.security.web.csrf.CookieCsrfTokenRepository; // 以 Cookie 方式提供 CSRF Token
...
//
//    // ===================== 常數：Cookie 名稱 =====================
//    public static final String ACCESS_COOKIE = "ACCESS_TOKEN";   // Access Token 放這顆 Cookie（HttpOnly）
//    public static final String REFRESH_COOKIE = "REFRESH_TOKEN"; // Refresh Token 放這顆 Cookie（HttpOnly）
//    
//    // ⏱️ Token 存活時間（TTL）：Access Token 短效，Refresh Token 長效
//    // java.time.Duration (Java 8+) not Spring or Spring Security-specific
//    public static final Duration ACCESS_TTL  = Duration.ofMinutes(15); // ⏳ Access 15 分鐘：降低外洩風險
//    public static final Duration REFRESH_TTL = Duration.ofDays(7);     // 🗓️ Refresh 7 天：用來換新 Access
//
...
//                // STATELESS
//                	// 永遠不使用 Session，適合*純* REST API + JWT 架構（完全林）
...
//        	
//        // exceptionHandling -> 例外處理 -> 配合 Spring Security 的 Filter Chain 如果認證獲授權失敗，框架會都出 Security 相關例外
...
//                        new LoginUrlAuthenticationEntryPoint("/front-end/login"), // 導向 /login 頁面
//                        new AntPathRequestMatcher("/front-end/**")
//                )
//                .defaultAuthenticationEntryPointFor(
//                		new LoginUrlAuthenticationEntryPoint("/back-end/login"),
...
//                		res.sendRedirect("/back-end/exhibitor_login");
//                	}
//                })
//                
...
//                .hasRole("MEMBER")
//                .requestMatchers("/back-end/exhibitor_login", "/front-end/exhibitor_register") // 目前其實沒有 back-end login
//                .permitAll()
...
//                .authenticated()
//                // .requestMatchers("/api/back-end/**", "/api/platform/**")
//                // .authenticated() // 甚至需要是 .hasRole("EXHIBITOR") 或 .hasAuthority(...)
//                // .requestMatchers("/api/front-end/protected/**") // 任何需要保護的 api 例如 -> /api/protected/front-end/cart/addCartItem...
//                // .authenticated() // 甚至需要是 .hasRole("PLATFORM") 或 .hasAuthority(...)
...
//
//        // ========= 掛上自訂的 JWT Cookie Filter（放在 UsernamePass	wordAuthenticationFilter 之前） =========
...
//                .logoutUrl("/api/auth/logout")                         // 指定登出端點（前端呼叫這個）（預設 /logout 登出，並清掉 session，此處改為 api 風格的 logout）
//                .addLogoutHandler(new CookieClearingLogoutHandler(     // 透過 Handler 清除 Cookie
//                        ACCESS_COOKIE, REFRESH_COOKIE                  // 兩顆 Token Cookie 一起清
//                ))
...
//        		// method: "POST",
//        		// credentials: "include" // 瀏覽器的行為設定，用來告訴 瀏覽器要不要附帶憑證（credentials）。
//        	// });
