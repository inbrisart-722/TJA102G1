	...
	getVerificationCodeBtn.addEventListener("click", (e) => {
		...
		check_member(email)
			.then((isMember) => {
				// 如果是會員，顯示錯誤訊息並停止後續流程
				if (!isMember) {
					emailError.textContent = "此信箱尚未註冊成為會員哦！";
					emailInput.classList.add("is-invalid");
					getVerificationCodeBtn.disabled = false;
					// 使用 Promise.reject() 停止 Promise 鏈，將控制權交給 .catch
					// 這樣後面的 .then 就不會執行
					return Promise.reject(new Error("check-if-member: Not a member"));
				}
			...
			.then(checkIfSendable => {
				...
				// 可以發驗證信了
				const send_data = { email, authType: "FORGOT_PASSWORD" };
				return csrfFetch("/api/front-end/verif/send-verif-code/forgot-password", {
					method: "POST",
			...
			.catch((error) => {
				...
				if (error.message.startsWith("您還需要等待")) {
				 ...
				 } else if (error.message !== "check-if-member: Not a member") {
				   console.error("Error:", error);
