...
function toggleCheckoutButton(enabled) {
	...
	const payBtn =
		document.querySelector("#send_payment")
	if (!payBtn) return;
	...
	if (enabled) {
		...
		payBtn.style.pointerEvents = "none"; // 根本就點不到的意思
...
document.addEventListener("click", (e) => {
	...
	let cartItemIds = [];
	all_icon_checked.forEach((i) => {
		let tr_el = i.closest("tr.cart_item_row");
		let cartItemId = tr_el.dataset.cartItemId;
